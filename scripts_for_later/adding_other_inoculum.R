...
# define the bad conditions
rejected_conditions = c("Mouse_P1_018", "Inoculum")

# prune them from the design matrix and the counts matrix
...
colnames(data_CPM) <- factor(exp_design[,  paste(media, gDNA_source, growth_condition, rep, sep = "_")])
...
melted_results <- melted_results[FDR != 1]
...
plot(-log10(FDR)~medLFC, data = median_melted_results[condition == "mouse_pellet_10x_inoculum_dilution - inoculum_pellet_t0"], pch = 20, main = "Median: mouse_pellet_10x_inoculum_dilution - inoculum_pellet_t0")
abline(v = median_melted_results[type == "control" & condition == "mouse_pellet_10x_inoculum_dilution - inoculum_pellet_t0"]$medLFC)
