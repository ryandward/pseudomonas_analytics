...
for i in *R1*gz; do
	echo seal.sh overwrite=t in=$i k=20 mm=f ambiguous=toss outm=stdout.fa hdist=1 trd=t rename fbm int=f ref=GuidesOnly.fasta \| awk \''$0~">"'\' \| sed \''s/>//g'\' \|  awk \''{
		winner = "Unknown";
		score = 0;
		for (i = 2; i <= NF; i++) {
			split ($i, parts, "=");
			if (parts[2] > score) {
				...
				winner = parts[1]; }}
				print $1, winner;
				delete parts; }'\' \> ${i%%_S*_R1_001.fastq.gz}.guides.tsv
done > map_it_all.sh;

for i in *R2*gz; do
  echo seal.sh overwrite=t in=$i k=20 mm=f ambiguous=toss outm=stdout.fa hdist=1 trd=t rename fbm int=f ref=PromotersOnly.fasta \| awk \''$0~">"'\' \| sed \''s/>//g'\' \|  awk \''{
		winner = "Unknown";
		score = 0;
		for (i = 2; i <= NF; i++) {
			split ($i, parts, "=");
			if (parts[2] > score) {
				score = parts[2];
				winner = parts[1]; }} print $1, winner;
				delete parts; }'\' \> ${i%%_S*_R2_001.fastq.gz}.promoters.tsv
done >> map_it_all.sh;
