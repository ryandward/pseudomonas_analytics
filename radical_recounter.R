...
exp_design[
	, verbose := paste(media, gDNA_source, growth_condition, rep, sep = "_")]

exp_design[
	, verbose_level := paste(media, gDNA_source, growth_condition, sep = "_")]

# discard all levels without replicates
exp_design <-
	exp_design[verbose_level %in% exp_design[
		, .N, by = verbose_level][
			N > 1, verbose_level]]
...
all_counts_round1 <- 
	fread(
		"all_counts_seal_round1_recount.tsv.gz", 
		header = FALSE, 
		col.names = c(
			"count", 
			"condition", 
...
all_counts <- rbind(all_counts_round1, all_counts_round2)
...
ineffective_guides <- results_prelim[type == "knockdown" & FDR > 0.25]
...
pheatmap(cor(
	data.matrix(
		dcast(
			exp_design[, .(condition, verbose)][
				CPM_melted[!spacer %in% ineffective_guides$name], 
				on = .(condition)], 
			spacer ~ verbose, value.var = "CPM"))[, -1]), 
	col = plot_colors,
	main = "CPM correlation after ineffective guides removed")
...
pheatmap(cor(
	data.matrix(
		dcast(
			...
			spacer ~ verbose, value.var = "CPM"))[, -1]), 
	col = plot_colors,
	main = "CPM correlation of only ineffective guides")
...
melted_results <- melted_results[!is.na(FDR) & !genes %in% ineffective_guides$name]
