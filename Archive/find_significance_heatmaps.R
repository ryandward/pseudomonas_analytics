...
groups_dt <- data.table(groups)
mouse_groups <- groups_dt[groups %like% "mouse"]

contrast <- makeContrasts(
	contrasts = c(
		"mouse_infection_t1_plate - mouse_inoculum_t0_pellet",
		"mouse_infection_t2_plate - mouse_inoculum_t0_pellet"
	), 
	levels = permut)
...
melted_results[FDR > 0.25, logFC := 0]

structured_results <- data.table::dcast(melted_results, guide ~ promoter + condition, value.var = "logFC", fill = 0)

plot_matrix <- data.matrix(structured_results[, -1])
...
plot_colors <- c(
	colorRampPalette(c("#ba000d", "white"))(break_halves), 
	colorRampPalette(c("white", "#007ac1"))(break_halves))

plot_colors[match(0, breaks)] <- "white"

to_plot_title <- paste("logFC for Mouse Libraries")
...
structured_results_for_export <- dcast(melted_results, FDR + genes ~ condition, value.var = "logFC")
